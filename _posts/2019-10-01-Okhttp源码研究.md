...
从这个最简单的demo去理解okhttp的内部运行机制。

首先从源码中OkHttpClient的注释来观察一下OkHttpClient
```java
...
```
OkHttpClient这个类有点总管的意思，负责发送请求，读取回复信息。且推荐重用该对象，因为每个OkHttpClient都有自己的连接池和线程池。
...
```
OkHttpClient中有很多与网络请求相关的属性，这里着重说明一下Dispatcher任务调度器，Dispatcher中有三个队列：准备执行的请求队列 readyAsyncCalls、正在运行的请求队列 runningAsyncCalls；一个正在运行的同步请求队列 runningSyncCalls
```java
